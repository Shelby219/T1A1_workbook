...
### Intellectual property, copyright, and acknowledgement.
  ...
  [Link 5](https://www.ipaustralia.gov.au/ip-for-digital-business/develop/ownership)

 ### Aggressive sales and marketing practices designed to mislead and deceive consumers
  Referencing the ACCC (Australian Competition & Consumer Commission) they quote ‘it is illegal for a business to make any statements that are incorrect or likely to create a false impression’. This includes media advertising, product packaging and statements made verbally by a business representative. Two stand out bad advertising practices are bait advertising and wild claims (puffery). Bait advertising is an aggressive advertising practice of claim specific prices for goods to services that are ultimately not available or only in small quantities. (Link 6, Screen Shot 1)  Puffery is the term used for absurd or vague claims about a product or services that would otherwise would not be taken seriously.(Link 6, Screen Shot 2) Additionally, advertisements with fact guessing, information omittance, ambiguous statements, confusing jargon and false promises are all clear examples of bad marketing practices. (Link 6, Screen Shot 3) More important is what the business should do with correct advertisement. Presenting information to customers about a product or service should in always be current and correct information, using simple language, confirming the overall impression is accurate, backing up claims with facts and evidence and noting any limitations or exceptions. (Link 6, Screen Shot 3) For an IT professional these are all important factors. The IT company would ensure the information was correct and current, by for example ensuring the developers can deliver what is portrayed, that they have the skill set, knowledge and resources to fulfil what is advertised. They would ensure the marketing is clear and in simple terms that the audience can understand, refraining from using abnormal IT terms that the customer is unlikely to understand. An example of a limitation or exception could be the company not outlining contingencies in project time frames, that would need to be subject to review or other factors. Advertising unattainable time frames of development projects would be bad practice. Any business offering goods or services to customers, ACCC protects customers of these businesses, so proper advertising and marketing practices are of upmost importance. 
...
## Q8 Explain control flow, using an example from the Ruby programming language
 ...
 [Link 1](http://zetcode.com/lang/rubytutorial/flowcontrol/)
...
## Q12 	Identify and explain the error in the following code that is preventing correct execution of the program
  ...
  The .to_i method needed to be added above as gets returns a string, so the string needed to be converted into an integer in order for mathematical calculations to work.
...
## Q13 The following code looks for the first two elements that are out of order and swaps them; however, it is not producing the correct results. Rewrite the code so that it works correctly.
    ...
    *********************************************** 
    ##The original code does not work as the last two lines of code override each other which results in both equalling 19.
...
#### b. the full list of allergies.	
    ...
    class Allergies
        ...
        ALLERGIES_S = {"eggs" => 1, "peanuts" => 2, "shellfish" => 4, "strawberries" => 8, 
      ...
      def allergic_to?(allergy)
        @total_score & ALLERGIES_S[allergy] != 0
      end

      def list
        print_list = ALLERGIES_S.keys.select { |allergy| allergic_to?(allergy) }
        puts "#{@name} is allergic to #{print_list.join(", ")}."
      end

    end

    tom_allergies = Allergies.new("Tom", 34)
    bill_allergies = Allergies.new("Bill", 41)
    puts tom_allergies.allergic_to?("peanuts") #true
    puts tom_allergies.allergic_to?("eggs") #false
    puts tom_allergies.list #peanuts, chocolate
    puts bill_allergies.list
